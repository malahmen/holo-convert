...
# Input format for md→pdf/docx. gfm_auto_identifiers makes pandoc generate
# GitHub-style heading IDs (numbers kept: "1.5 Glossary" → "15-glossary") so
# internal anchor links authored for GitHub/Starlight resolve to the right
# heading. Plain `markdown` strips leading numbers/punctuation, so those links
# point at nothing and Word/Pages jump to the first page.
MD_INPUT_FORMAT="markdown+gfm_auto_identifiers"
CHECK_LINKS=true          # warn about internal anchor links that match no heading
FIX_LINKS=false           # --fix-links: auto-rewrite confident fixes into the SOURCE
...
# -----------------------------------------------------------------------------
# Turn pandoc's image alt text into a native, auto-numbered Word caption
# ("Figure N: …" via a SEQ field + the built-in Caption style) and clear the
# duplicated alt-text description off the figure. No-op when there are no
# captioned figures. Skipped cleanly if python3 is missing.
# -----------------------------------------------------------------------------

caption_docx_figures() {
    local docx="$1"
    command -v python3 &>/dev/null || return 0
    local script; script="$(resolve_docx_asset caption_figures.py)"
    [[ -n "$script" ]] || return 0
    python3 "$script" "$docx"
}

# -----------------------------------------------------------------------------
# Reconcile internal anchor links ([x](#id)) so they resolve in the output.
#
# An anchor that matches no heading id silently jumps to the top of the document
# in Word/Pages. Slug rules differ subtly between tools (and pandoc versions) —
# e.g. a heading with '&' or ' - ' can yield a different hyphen count than the
# GitHub/Starlight slug the author linked against. Using pandoc's own AST we spot
# these and, for a confident near-miss (same slug once hyphen-runs are
# collapsed), rewrite the anchor to the id pandoc actually generates.
#
# By default the fix is applied only to the WORKING COPY (the .tmp.md we hand to
# pandoc), so the OUTPUT resolves while the SOURCE is left as authored — its
# anchors may legitimately follow another tool's slugs (e.g. a live docs site).
# --fix-links additionally writes the confident fixes back into the source.
# Uncertain matches are only ever reported. No-op without python3/pandoc; never
# fatal — a bad link shouldn't fail a build.
#   $1 — working-copy (.tmp.md) path      $2 — original source path
# -----------------------------------------------------------------------------

reconcile_internal_links() {
    [[ "$CHECK_LINKS" == true ]] || return 0
    local tmp="$1" src="$2"
    command -v python3 &>/dev/null && command -v pandoc &>/dev/null || return 0
    local script; script="$(resolve_docx_asset check_links.py)"
    [[ -n "$script" ]] || return 0
    # Reconcile the working copy (post pre-passes) so the OUTPUT resolves. The
    # copy is what pandoc converts, so its heading ids are authoritative here.
    python3 "$script" --reconcile "$tmp" || true
    # --fix-links also persists confident fixes to the SOURCE, resolved against
    # the source's own headings (which pre-passes never touched).
    [[ "$FIX_LINKS" == true ]] && python3 "$script" --fix "$src" || true
}
...
# -----------------------------------------------------------------------------
# Build the pandoc --resource-path for a md→* conversion.
# $1 — original source path. Relative resources (images, includes) in markdown
# resolve against the SOURCE file's directory; but we hand pandoc a working
# .tmp.md in ./output/, so without help it resolves them from there instead.
# Returns a `:`-separated list: source dir first, then the output dir and CWD as
# fallbacks (so absolute paths and output-relative assets still work). Prints
# nothing if the source directory can't be resolved.
# -----------------------------------------------------------------------------
...
convert_md_to_pdf() {
    ...
    # See convert_md_to_docx: relative images resolve against the source dir,
    # not the ./output/ .tmp.md we hand to pandoc.
    local res_path
    res_path="$(resolve_resource_path "$name_source")"
...
convert_md_to_docx() {
    ...
    # Images in the source use paths relative to the SOURCE file, but we convert
    # a .tmp.md living in ./output/, so pandoc would resolve them from the wrong
    # directory. Point --resource-path at the original source's directory so
    # relative images (and other resources) still resolve.
    local res_path
    res_path="$(resolve_resource_path "$name_source")"
    [[ -n "$res_path" ]] && pandoc_args+=(--resource-path="$res_path")
    ...
    if [[ "$CONCAT" == "true" && "$SOURCE_FORMAT" == "md" ]]; then
        ...
        while IFS= read -r input_file; do
            ...
            if [[ "$SOURCE_FORMAT" == "md" ]]; then
                ...
                # heading (e.g. — → -) and thus its slug, so anchors must be
                # matched against the FINAL working copy pandoc will convert.
                reconcile_internal_links "$tmp_file" "$input_file"
            fi
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --fix-links)           FIX_LINKS=true; shift ;;
            --no-check-links)      CHECK_LINKS=false; shift ;;
